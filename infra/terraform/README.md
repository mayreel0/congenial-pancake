...
- AWS CLI configured locally (`aws configure` or `AWS_PROFILE`) with credentials that can create VPC/EC2/RDS/ALB/IAM/ECR/ACM/Route53 resources.
...
cd infra/terraform
terraform init
terraform apply -target=aws_ecr_repository.api   # just the ECR repo
...
In another terminal, with the tunnel still open:

```bash
DB_PASSWORD=$(aws secretsmanager get-secret-value --secret-id "$(terraform output -raw rds_secret_arn)" --query SecretString --output text | python3 -c "import sys,json;print(json.load(sys.stdin)['password'])")
cd ../../apps/api-server
DATABASE_URL="postgres://onseol:${DB_PASSWORD}@localhost:5433/onseol" pnpm db:migrate
```

Same tunnel works for `pnpm db:studio` if you want to inspect data directly.
