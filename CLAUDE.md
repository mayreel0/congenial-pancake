...
- Do not push directly to `main` or `v1`. All changes go through a work branch and a PR with `--base v1`.
- Linear is the source of truth: find or create the issue first (no duplicates, never invent an ID); branch `<type>/<issue-id>-<short-description>`, PR title `<type>: <issue-id> <description>`, PR body starts with `Linear: <issue-id>` and separates verified checks from remaining ones; move to `In Progress` when starting and `Needs User` for blocking decisions; `In Review`/`Done` are set automatically by PR open/merge, never manually; never merge without the user's explicit approval.
- Follow the PR review workflow, review publishing rules, and review feedback response rules in `AGENTS.md`. Those shared sections are identical across projects; repo-specific details go in Project-Specific Settings.
