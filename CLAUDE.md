...
- Linear is the source of truth: find or create the issue first (no duplicates); branch `<type>/<issue-id>-<short-description>`, PR title `<type>: <issue-id> <description>`; status `In Progress` → `In Review` → `Done` only after review+merge; blocking user decisions → `Needs User`.
