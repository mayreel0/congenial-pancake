...
`SwaggerModule` is wired in `main.ts`, mounted at `SWAGGER_DOCS_PATH` (env var, defaults to a non-predictable slug — **not** `/docs` or `/api-docs`, deliberately, see docs/decisions/2026-08-26-onseol-openapi-decisions.md). Schemas are auto-inferred from DTO classes at build time via the `@nestjs/swagger` NestJS CLI plugin (`nest-cli.json`'s `compilerOptions.plugins`) — this reads both TS types and `class-validator` decorators (`@MinLength()` etc. show up as OpenAPI constraints), so DTOs don't need manual `@ApiProperty()` decoration. This only runs through `nest build`/`nest start` (the CLI), not plain `tsc --noEmit` — don't expect `pnpm --filter api-server typecheck` to exercise it. Only `@ApiTags('...')` on each controller is manual (grouping needs a human-chosen name, not inferable) — add it to any new controller, or its routes land ungrouped in the UI. This is kept on in production (unlike a typical internal-only setup), since this API has no other consumer-facing docs yet — see the decision doc for why a hard-to-guess path is the interim measure instead of the real host-based subdomain separation eventually planned once actual deploy infra exists.
