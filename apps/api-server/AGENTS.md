...
`SwaggerModule` is served on its **own port** (`SWAGGER_PORT`, default 8081) — not a path on the main API — via a second, otherwise-empty Nest app (`SwaggerDocsModule` in `main.ts`) that hosts the pre-generated OpenAPI document at `/`. Same reasoning as `apps/admin` getting its own port instead of a path inside `apps/web`: once the port itself is the separation, the path doesn't need to be obscure too. See `docs/decisions/2026-08-26-onseol-openapi-decisions.md` (originally shipped as a hard-to-guess path on the main port; moved to a separate port once the user asked whether that was possible — no real deploy infra yet, so a second port on the same process was the actually-achievable version of "give it its own address," same idea as the subdomain-separation goal that's parked until deploy infra exists).
