...
`SwaggerModule` is served on its **own port** (`SWAGGER_PORT`, default 8081) — not a path on the main API — via a second, otherwise-empty Nest app (`SwaggerDocsModule` in `src/openapi.ts`) that hosts the pre-generated OpenAPI document at `/`. Same reasoning as `apps/admin` getting its own port instead of a path inside `apps/web`: once the port itself is the separation, the path doesn't need to be obscure too. See `docs/decisions/2026-08-26-onseol-openapi-decisions.md` (originally shipped as a hard-to-guess path on the main port; moved to a separate port once the user asked whether that was possible — no real deploy infra yet, so a second port on the same process was the actually-achievable version of "give it its own address," same idea as the subdomain-separation goal that's parked until deploy infra exists).

`pnpm --filter api-server start:swagger` (or `start:swagger:watch`) runs `src/swagger-only.ts` instead of `src/main.ts` — same OpenAPI document (built from `AppModule`'s real controllers via the shared `buildOpenApiDocument()` helper in `src/openapi.ts`), but `app.listen(PORT)` is never called, so the real API isn't reachable at all from that process — only `SWAGGER_PORT` opens. Useful for browsing docs without standing up the full live app. It still needs a valid `.env` (same as `main.ts` — `AppModule` still gets instantiated to discover the real routes) but never queries the DB.

Schemas are auto-inferred from DTO classes at build time via the `@nestjs/swagger` NestJS CLI plugin (`nest-cli.json`'s `compilerOptions.plugins`) — this reads both TS types and `class-validator` decorators (`@MinLength()` etc. show up as OpenAPI constraints), so DTOs don't need manual `@ApiProperty()` decoration. This only runs through `nest build`/`nest start` (the CLI), not plain `tsc --noEmit` — don't expect `pnpm --filter api-server typecheck` to exercise it. Only `@ApiTags('...')` on each controller is manual (grouping needs a human-chosen name, not inferable) — add it to any new controller, or its routes land ungrouped in the UI. Kept on in production (unlike a typical internal-only setup), since this API has no other consumer-facing docs yet.
