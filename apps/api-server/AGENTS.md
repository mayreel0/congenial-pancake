...
## Nicknames (opt-in reveal, not built yet)

`users.nickname` (nullable, **not unique**) lets a member set a self-chosen display name — `POST /auth/nickname`, session-required, 1-20 chars, can't be whitespace-only. Two people can pick the same nickname on purpose (no username-hunting friction); they're told apart by a discriminator derived from `id` (`src/users/nickname-discriminator.ts` — last 4 hex chars of the UUID, uppercased, no separate column, always computed and returned even when `nickname` is null). **Nothing reads or exposes this yet** — `requests`/`replies` have no `anonymous`/reveal flag, so nicknames aren't shown anywhere on a post. That's deliberately a separate, larger round (touches `feed-author-slots.ts`'s per-thread anonymous labeling, response DTOs across `/today`/`/answer`/`/read`/`/me`) — see `docs/decisions/2026-08-28-onseol-nickname-decisions.md` for the full design (opt-in **per post**, not an account-wide switch, so revealing a nickname once never retroactively links past anonymous posts).
