...
## Account area: 내 정보 / 내 기록 / 설정 (split, not one /me page)

Originally one `/me` page held identity info, the nickname section, and the answer log together. Split (2026-08-29, user feedback: "내가 남긴 답변, 닉네임 수정을 모두 /me 에 넣지않고 분리") into three routes, each its own thin page + self-contained section component(s) that call `useAuth()`/query hooks themselves (no props from the page, matching the pattern already established by `MyAnswerLogSection`):

- `/me` (`app/me/page.tsx`) — **내 정보**: just email + joined date. Nothing else lives here anymore.
