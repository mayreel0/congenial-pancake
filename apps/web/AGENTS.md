...
- `/records` (`app/records/page.tsx`) — **내 기록**: `MyAnswerLogSection` (`app/records/components/`, moved from `app/me/components/`) — "내가 남긴 답변" (replies the viewer gave to others). A symmetric "내가 작성한 고민" (the viewer's own posted requests + the replies they received) is planned but not built yet — see backend `GET /requests/mine` work tracked separately.
