...
describe("isStandaloneApp", () => {
  ...
  it("is false in a plain browser tab", async () => {
    ...
    const { isStandaloneApp } = await loadPush();

    expect(isStandaloneApp()).toBe(false);
  });

  it("is true when launched with display-mode: standalone", async () => {
  ...
  it("is true for an iOS home-screen app via navigator.standalone", async () => {
    stubDisplayMode(false);
