...
// Shared between OAuthButton (/login's full-width buttons) and
// LinkedProvidersSection (/me's compact icon tiles) — the icons
// themselves are brand-guideline-sensitive assets that shouldn't be
// redrawn per call site, and the brand colors are the same wherever a
// provider shows up. No shared "label" here — each call site needs a
// different string (a full call-to-action on the login button vs. a
// short name for the compact tile's aria-label), so that stays local.
export const OAUTH_PROVIDER_STYLES: Record<
  OAuthProviderName,
  { className: string; Icon: () => React.JSX.Element }
> = {
  google: {
...
export const OAUTH_PROVIDER_NAMES_KO: Record<OAuthProviderName, string> = {
  google: "Google",
  kakao: "카카오",
  naver: "네이버",
};
