...
Three routes: `/` (`AdminReview.tsx` — "신고 검토": hidden requests/replies, restore/soft-delete), `/settings` (`SettingsReview.tsx` — the DB-backed tunable limits, see `apps/api-server/AGENTS.md`'s "DB-backed settings" and `docs/decisions/2026-08-26-onseol-db-backed-settings-decisions.md`), and `/accounts` (`AccountsReview.tsx` — "계정": issues a one-off password-reset link for an OAuth-only account, see `docs/decisions/2026-08-27-onseol-oauth-password-reset-decisions.md`; no email sending, the link is just shown on screen to copy). `app/components/AdminNav.tsx` is the shared nav (신고 검토/설정/계정 tabs + logout) — it was deliberately *not* added until a second section existed (see the original `/admin` decision's narrow-scope reasoning), and each subsequent tab followed the same rule: add only when a real new section is being built, not preemptively. `/accounts` has no backing GET query (unlike the other two pages), so its `useAccountsAdmin` hook can't precheck "forbidden" the way `useAdminSettings`/`useAdminReview` do — a non-admin session only discovers that from the mutation's own 401/403, shown inline as `issueError` rather than via `AdminStatusGate`'s dedicated forbidden branch.
