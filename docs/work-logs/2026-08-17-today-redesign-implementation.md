...
- 모바일에서는 입력창이 손이 닿기 쉬운 하단 근처에 오도록 배치하되, 키보드/safe-area 복잡도가 큰 fixed bottom 입력창은 아직 쓰지 않는다.
...
- 모바일 `/today`는 화면 전체를 세로 grid로 쓰고 `content-end`로 핵심 입력 흐름을 아래쪽에 둔다. 태블릿/PC에서는 다시 중앙 정렬한다.
