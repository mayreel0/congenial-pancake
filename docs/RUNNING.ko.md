...
네이버 OAuth 로그인은 `NAVER_CLIENT_ID`와 `NAVER_CLIENT_SECRET`이 모두 설정된 경우에만 활성화됩니다. MVP 회원가입은 이메일/비밀번호와 닉네임을 받으며, 닉네임 칸에는 중복되지 않는 랜덤 닉네임을 먼저 제안합니다. 네이버 OAuth 신규 사용자는 첫 로그인 직후 닉네임 설정 화면에서 랜덤 닉네임, 네이버 이름 가져오기, 직접 입력 중 하나를 선택한 뒤 서비스를 계속 이용합니다. 이메일과 닉네임 중복은 서버에서 막습니다. 제재된 사용자는 새 계정으로 우회하는 대신 기존 계정의 제한 해제를 요청하는 정책으로 둡니다.
