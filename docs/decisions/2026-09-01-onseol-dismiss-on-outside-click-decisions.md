...
3. **`ActionConfirmDialog`에 배경(backdrop) 클릭 = 취소 추가** — 훅을 다이얼로그에도 적용할 수 있냐는 질문에서 시작. 안쪽 박스에 ref를 걸고 `onDismiss`를 `onCancel`로 연결하면 그대로 재사용 가능했지만, 지금까지 배경 클릭이 무반응이었던 걸 "취소"로 바꾸는 실제 동작 변경이라 확인 필요 → 사용자가 "네, 배경 클릭 = 취소로 추가" 확인. `ActionConfirmDialog`가 `packages/ui`에 있어 `apps/web`+`apps/admin`의 모든 확인 다이얼로그(신고 확인, 방문자 설정 저장 확인, admin 복구/삭제 확인 등)에 한 번에 적용됨.

## 검증

Storybook(`apps/storybook-app`, 포트 6006)에서 실제 브라우저로 확인:
- `MoreMenu` 기본 스토리: 클릭으로 메뉴 열림, 바깥 클릭으로 닫힘.
- `ActionConfirmDialog` 기본 스토리: 배경 클릭 시 `onCancel` 정확히 1회 호출(Storybook Actions 패널로 확인), 박스 안쪽(메시지 텍스트 영역) 클릭 시 `onCancel` 호출 안 됨(오탐 없음 확인).
