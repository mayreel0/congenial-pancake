...
## 추가: 토글은 원래 위치로 되돌리고, 저장 플로우만 페이지 전체에서 공유 (같은 날, 사용자 정정)

바로 위 라운드에서 "하나로 합치기"를 토글까지 한 카드로 물리적으로 합치는 것으로 해석하고 그렇게 구현했는데, 사용자가 이는 의도와 다르다고 정정: "살짝 의도랑 다른데.. 닉네임 공개를 공개 설정에 넣으면 헷갈리잖아. 원래 위치로 놔두고 그냥 설정 변경을 전체적으로 하라고 이해가 안돼? 페이지 전채적으로 설정 변경을 한번에 할거라고 공개 설정만 묶는게 아니라." — 즉 토글은 각자 원래 섹션(닉네임 공개는 `NicknameSection`, 나머지 3개는 프로필 공개 설정 섹션)에 그대로 두고, **저장/취소/확인 다이얼로그라는 메커니즘만** 페이지 전체에서 하나로 공유하라는 뜻이었음. 공유 저장/취소 버튼을 어디에 둘지 확인한 결과 "페이지 하단에 고정 바"로 확정(각 카드 아래 중복 배치안은 기각).

- `VisibilitySettingsSection`을 `ProfileVisibilitySection`으로 되돌리고(닉네임 공개 토글 제거, 원래의 3개 토글만), `NicknameSection`에 닉네임 공개 토글을 다시 추가.
- `VisibilityDraftProvider`(`app/me/components/`)를 새로 도입 — `draft`/`isDirty`/`setField`를 React Context로 제공하고, 두 섹션이 각자 `useVisibilityDraft()`로 자기 필드만 읽고 쓴다. 저장/취소 버튼과 `ActionConfirmDialog`는 이 Provider 안에서 한 번만 렌더링되며, `isDirty`일 때만 나타나는 `position: fixed` 하단 바로 구현 — 평소엔 화면에 아무 버튼도 없다가 뭔가 바뀌는 순간 나타남.
- `MePage`가 `user`가 non-null임을 확정한 뒤에만 `VisibilityDraftProvider`로 두 섹션을 감싸므로, Provider 자체는 `user: CurrentUser`를 필수 prop으로 받아 null 처리를 할 필요가 없음.
