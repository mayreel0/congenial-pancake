...
- 온설은 사용자가 오늘 힘들었던 일, 칭찬받고 싶은 일, 위로받고 싶은 일을 짧게 남기고 다른 사용자가 현실적이고 담백한 답장을 남기는 Korean-first 서비스다.
...
- 첫 MVP 인증 모델: cookie session, JWT, 외부 provider token 검증 중 무엇을 쓸지.
...
제품, UX, 백엔드, 인프라, 신고/필터 정책에 의미 있는 tradeoff가 있으면 에이전트는 추천안과 이유를 먼저 제시하고 사용자 확인을 받은 뒤 확정한다.
