...
이번 라운드는 백엔드 변경이 없으므로(스키마는 이미 있음) 문서 → 프론트엔드 2개 PR로 진행. `v1` 기반, 순차 — **#132(문서, 머지) → #133(프론트엔드, 머지)**.

## 재검토 (2026-09-02, PR #133 진행 중) — 계획했던 "제출+blur" 규칙을 실제로는 두 갈래로 나눔

첫 구현은 계획대로 6개 폼 전부에 "제출+blur, touched 이후 실시간" 규칙을 똑같이 적용했으나, 두 번의 사용자 피드백으로 최종 형태가 달라졌다.

**1차 재검토 — `/today`·`/answer` 컴포저 3곳은 에러 텍스트 자체를 제거.** "today 에는 빈값에 대해 에러 메시지를 띄울 필요가 없는 동작인데 왜 거기다가 넣었대?" — 챗 컴포저는 아직 아무것도 안 쓴 빈 상태가 "잘못"이 아니라 그냥 초기 상태인데, blur마다 "내용을 입력해주세요"를 띄운 건 이미 있던 "보내기" 버튼 비활성화만으로 충분한 신호를 굳이 텍스트로 한 번 더 나무라는 과잉 적용이었음을 인정 — "제출+blur" 규칙을 로그인/닉네임처럼 실제 값을 입력하다 틀리는 구조적 폼에 맞춰 확정한 거였는데, 컴포저 3곳까지 기계적으로 똑같이 적용한 게 원인. `RequestComposer`/`ReplyComposer`/`AnswerComposer`는 `useFieldValidation`/에러 텍스트/`onBlur`를 전부 제거하고, `parseFieldErrors` 결과로 제출 버튼만 비활성화하는 원래 방식으로 되돌림 — 스키마 기반 게이팅 자체는 남겨서 향후 `.refine()` 규칙이 추가돼도 계속 막아줌(위 "확장성" 참고).

**2차 재검토 — 남은 3개 폼(로그인/회원가입, 닉네임, 비밀번호 재설정)도 blur 검증을 제거, 제출 시에만 검증.** "블러 동작 하나하나 생각해서 말해주기 귀찮네" — 필드마다 blur 타이밍이 맞는지 따로 판단해달라고 요구하는 게 번거롭다는 피드백으로, blur 검증을 전부 빼고 제출 시 한 번에 `touchAll`하는 것으로 단순화. 텍스트 표시 자체(빨간 글씨 에러 메시지)는 유지 — 별도로 물어본 "border만 쓸지 텍스트도 유지할지"는 텍스트 유지로 확정됐음(왜 틀렸는지 설명이 border 색만으론 전달 안 됨). `useFieldValidation`에서 개별 필드용 `touch(field)`를 제거하고 `touchAll(fields)`만 남김.

결과적으로 최종 규칙은 폼 종류에 따라 갈린다: **구조적 폼(로그인/회원가입/닉네임/비밀번호 재설정)은 제출 시에만 검증 + 에러 텍스트 표시, 챗 컴포저(고민/답변 작성 3곳)는 검증 결과로 제출 버튼만 막고 에러 텍스트는 아예 없음.** 처음 계획했던 "6곳 전부 동일 규칙"은 폐기.

## 검증

- `pnpm --filter web test`: 134/134 (신규 `parseFieldErrors` 유닛 테스트 3개 포함, 컴포저 blur-에러 테스트는 재검토 과정에서 추가했다가 해당 동작 자체가 제거되면서 함께 삭제).
- `pnpm --filter web lint`, `pnpm --filter ui lint`, 양쪽 `tsc --noEmit` — 클린.
- 실브라우저(Chrome)로 `/today` `RequestComposer`에서 빈 값 blur 시 에러가 안 뜨는 것(1차 재검토 이후), 값을 입력하면 버튼이 즉시 활성화되는 것 확인. 로그인/닉네임/비밀번호 재설정 폼의 제출-시-검증(2차 재검토)은 코드 변경이 기계적이라(핸들러 제거) 별도 실브라우저 재검증 없이 테스트 스위트 통과로 갈음.
