...
- 오늘 경계는 서비스 레이어에서 KST 기준으로 계산해 `Date`로 레포지토리에 넘김 — 레포지토리는 순수 카운트만, 날짜 산술은 안 함(`RequestsRepository.findQueueCandidate` 패턴과 동일).
- `apps/api-server/src/common/kst-date.ts`에 `todayKstDateString()` 추가.
- 단일 객체 응답이므로 `@ZodResponse({ type: LandingStatsResponseDto })` 사용.
- **처음엔 `today`/`month`/`total` 3단이었다가 `month` 제거함** — 프론트 카드 레이아웃을 6칸→5칸으로 줄이면서(아래 "재검토" 참고) 안 쓰는 필드라 API에서도 같이 뺌. `kstMonthToDateRange()` 헬퍼도 같이 삭제.
...
- `ActivityStats.tsx` — **처음엔 오늘/이번달/전체 × 위로요청/답장 6칸 그리드 + `waitingForReply` 별도 텍스트로 만들었다가, 사용자 피드백으로 재검토함**: 각 카드 아래 붙어있던 예전 "짧게 남긴 이야기" 같은 설명 캡션이 없어지고 숫자만 나열되니 6칸이 오히려 휑해 보인다는 지적. "이번 달" 단을 통째로 빼고 **5칸(누적 위로요청/누적 답장 2칸 + 오늘의 위로요청/오늘의 답장/답변을 기다리는 글 3칸, 두 줄)** 으로 재구성 — `답변을 기다리는 글`도 별도 텍스트가 아니라 다섯 번째 카드로 편입. 로딩 중엔 `ui/Skeleton`.
...
- `LandingPage.tsx`의 `<main>`에 있던 `min-h-[calc(100vh-5rem)]`을 `lg:min-h-[...]`로 변경 — 데스크톱에서만 뷰포트 높이를 맞추고, 태블릿/모바일에서는 콘텐츠 높이를 그대로 따라가게 함. 같은 요소에 이미 있던 `lg:grid-cols-[1fr_420px]`/`lg:items-center`/`lg:py-12`와 같은 패턴. 나중에 footer가 추가되면 고정 높이가 더 어긋날 거라는 점도 이유.

## 검증

- `pnpm --filter api-server test`: 163/163 (새 `LandingService` 유닛 테스트 5개 포함, `month` 제거 후에도 그대로 통과).
- 로컬 Postgres에 실제 데이터 시딩 후 `curl localhost:3001/public/stats`, `/public/samples?limit=6`로 실제 값 확인 — `month` 필드가 있던 시점엔 KST 월 경계가 정확히 동작함도 확인했음(오늘이 9/1이라 8/29에 만든 글은 "이번 달"에서 정확히 제외됨), 필드 제거 후 남은 `today`/`total`도 재확인.
...
- 5칸 재구성 + 반응형 높이 수정 후에도 좁은 뷰포트(태블릿 폭)에서 실브라우저로 재확인 — 2+3 카드 레이아웃과 `lg:min-h-[...]`로 바뀐 뒤 모바일/태블릿 폭에서 더 이상 뷰포트 높이가 강제되지 않는 것 확인.
