...
- **KD9. 의사결정은 먼저 물어보고 확정한다.** 제품, UX, 기술스택, 백엔드, 인프라, 배포, 신고/필터, 작업 방식에 의미 있는 tradeoff가 있으면 추천안과 이유를 제시하고 사용자 확인을 받은 뒤 확정한다.
- **KD10. main/v1에는 직접 push하지 않는다.** 모든 변경은 작업 브랜치에서 커밋하고 PR로 남긴 뒤 검토를 거쳐 반영한다.
- **KD11. effective-doodle은 LLM wiki로 사용한다.** 프로젝트 진행 중 생기는 핵심 개념, 결정, 과정, 실수, 교훈은 나중에 Obsidian 기반 프로젝트 위키에 축적할 지식으로 취급한다.
...
- R27. 프로젝트 진행 중의 핵심 개념, 결정, 과정, 실수, 교훈은 LLM wiki에 남길 수 있도록 기록한다.
...
- 중요한 결정과 반복 방지용 교훈이 Project Wiki Mode를 통해 위키에 축적된다.
