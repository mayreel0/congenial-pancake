...
- `packages/api-client` — shared fetch wrapper + auth calls (`apiFetch`, `login`/`logout`/`fetchCurrentUser`). Named `api-client`, not `api`, to avoid colliding with `apps/api`'s own package name — see `packages/api-client/AGENTS.md`.
...
- `apps/storybook` — runs Storybook only; owns no components itself. Its `stories` glob scans component-adjacent story files wherever they actually live (`apps/web/app/**/*.stories.tsx`, `packages/ui/src/**/*.stories.tsx`) rather than requiring them to move here — see `apps/storybook/AGENTS.md` and `docs/decisions/2026-08-26-onseol-storybook-app-decisions.md`.
- `apps/api` — Nest.js backend. Run scripts via `pnpm --filter api <script>`. See `apps/api/AGENTS.md` for Nest-specific conventions (auth, DB, module boundaries).
