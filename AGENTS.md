...
- Linear: team `Development`, project `congenial-pancake`. Issue IDs look like `DEV-53`.
- Base branch: `v1`. `main` has unrelated history, and `gh pr create` defaults to it, so always pass `--base v1`.
...
- Commit messages and PR titles use the same `<type>:` prefix (`feat:`, `fix:`, `chore:`, `docs:`, `refactor:`, ...) for the same change. Before writing either, check the existing convention rather than assuming — `git log --oneline -10` for commit style, `gh pr list --state all --limit 10` for PR title style. Examples: branch `feat/DEV-31-login-session-expiry`, PR title `feat: DEV-31 Add session expiration handling`.
- CI runs `verify` (`.github/workflows/verify.yml`) and `check-title` (`.github/workflows/pr-title.yml`) on every PR. `check-title` rejects a title without `DEV-<number>` right after the type prefix; there is no exception for bot PRs, because none exist yet — add one when a dependency bot is introduced.
...
- Gemini review prompts are generated by the Build PR Review Prompt workflow (`.github/workflows/build-review-prompt.yml`, manual `workflow_dispatch`), which embeds `docs/review-criteria.md`. The user pastes the generated prompt into Antigravity IDE.
